IF OBJECT_ID('tbl_authors', 'U') IS NULL
BEGIN
...
BEGIN
    CREATE TABLE tbl_books (
        ...
        CONSTRAINT FK_books_author FOREIGN KEY (authorId) REFERENCES tbl_authors(id)
    );
END;
...
IF OBJECT_ID('tbl_stores', 'U') IS NULL
BEGIN
...
IF OBJECT_ID('tbl_bookBalance', 'U') IS NULL
BEGIN
    CREATE TABLE tbl_bookBalance (
        ...
        CONSTRAINT FK_bookBalance_Store FOREIGN KEY (storeId) REFERENCES tbl_stores(id),
        CONSTRAINT FK_bookBalance_Book  FOREIGN KEY (isbn)     REFERENCES tbl_books(isbn)
    );
END;

IF NOT EXISTS (SELECT 1 FROM tbl_bookBalance)
BEGIN
    INSERT INTO tbl_bookBalance (storeId, isbn, amountInStock) VALUES
        (1, '9780140441185', 12),
        (1, '9780140442038', 8),
        (1, '9780140449136', 5),
        (1, '9780811216876', 7),
...
IF OBJECT_ID('tbl_customers') IS NULL
BEGIN
...
IF NOT EXISTS (SELECT 1 FROM tbl_customers)
BEGIN
INSERT INTO tbl_customers (firstname, lastname, email, phone) VALUES
...
IF OBJECT_ID('tbl_orders') IS NULL
BEGIN
CREATE TABLE tbl_orders (
    ...
    CONSTRAINT FK_orders_customer FOREIGN KEY (customerId) REFERENCES tbl_customers(customerId),
    CONSTRAINT FK_orders_store FOREIGN KEY (storeId) REFERENCES tbl_stores(id)
);
END;

IF NOT EXISTS (SELECT 1 FROM tbl_orders)
BEGIN
INSERT INTO tbl_orders (customerId, storeId, totalAmount) VALUES
    (1, 1, 388.00),
    (2, 2, 219.00),
    (3, 1, 169.00),
    (4, 3, 448.00),
...
IF OBJECT_ID('tbl_orderDetails', 'U') IS NULL
BEGIN
